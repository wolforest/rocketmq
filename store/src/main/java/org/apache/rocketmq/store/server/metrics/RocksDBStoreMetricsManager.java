...
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_BYTES_ROCKSDB_READ;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_BYTES_ROCKSDB_WRITTEN;
    ...
    public static void init(Meter meter, Supplier<AttributesBuilder> attributesBuilderSupplier,
        ...
        bytesRocksdbWritten = meter.gaugeBuilder(GAUGE_BYTES_ROCKSDB_WRITTEN)
                ...
                .buildWithCallback(measurement -> {
                    measurement.record(((RocksDBConsumeQueueStore)messageStore.getConsumeQueueStore())
        ...
        rocksdbCacheHitRate = meter.gaugeBuilder(DefaultStoreMetricsConstant.GAUGE_RATE_ROCKSDB_CACHE_HIT)
                ...
                .buildWithCallback(measurement -> {
                    long newHitTimes = ((RocksDBConsumeQueueStore)messageStore.getConsumeQueueStore())
                            .getStatistics().getTickerCount(TickerType.BLOCK_CACHE_HIT);
                    long newMissTimes = ((RocksDBConsumeQueueStore)messageStore.getConsumeQueueStore())
