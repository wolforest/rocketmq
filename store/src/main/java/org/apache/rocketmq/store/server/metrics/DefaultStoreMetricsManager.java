 ...
 */
package org.apache.rocketmq.store.server.metrics;
...
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.COUNTER_TIMER_DEQUEUE_TOTAL;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.COUNTER_TIMER_ENQUEUE_TOTAL;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.DEFAULT_STORAGE_MEDIUM;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.DEFAULT_STORAGE_TYPE;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_STORAGE_DISPATCH_BEHIND;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_STORAGE_FLUSH_BEHIND;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_STORAGE_MESSAGE_RESERVE_TIME;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_STORAGE_SIZE;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_TIMER_DEQUEUE_LAG;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_TIMER_DEQUEUE_LATENCY;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_TIMER_ENQUEUE_LAG;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_TIMER_ENQUEUE_LATENCY;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_TIMER_MESSAGE_SNAPSHOT;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.GAUGE_TIMING_MESSAGES;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.HISTOGRAM_DELAY_MSG_LATENCY;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.LABEL_STORAGE_MEDIUM;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.LABEL_STORAGE_TYPE;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.LABEL_TIMING_BOUND;
import static org.apache.rocketmq.store.server.metrics.DefaultStoreMetricsConstant.LABEL_TOPIC;
...
public class DefaultStoreMetricsManager {
    ...
    public static void init(Meter meter, Supplier<AttributesBuilder> attributesBuilderSupplier,
        ...
        if (messageStore.getMessageStoreConfig().isTimerWheelEnable()) {
            ...
            initTimerMessageSnapshot(meter, messageStore);
            initTimerMessageSetLatency(meter);
        }
    }
    ...
    private static void initDispatchBehind(Meter meter, DefaultMessageStore messageStore) {
    ...
    private static void initTimerEnqueueLatency(Meter meter, DefaultMessageStore messageStore) {
    ...
    private static void initTimerDequeueLatency(Meter meter, DefaultMessageStore messageStore) {
        timerDequeueLatency = meter.gaugeBuilder(GAUGE_TIMER_DEQUEUE_LATENCY)
            ...
            .buildWithCallback(measurement -> {
                TimerMessageStore timerMessageStore = messageStore.getTimerMessageStore();
                measurement.record(timerMessageStore.getTimerState().getDequeueBehind(), newAttributesBuilder().build());
            });
    }
    ...
    private static void initTimerMessageSetLatency(Meter meter) {
    ...
    private static void initTimerMessageSnapshot(Meter meter, DefaultMessageStore messageStore) {
