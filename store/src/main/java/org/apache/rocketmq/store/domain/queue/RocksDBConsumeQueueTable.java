...
import org.apache.rocketmq.store.infra.rocksdb.ConsumeQueueRocksDBStorage;
...
public class RocksDBConsumeQueueTable {
    ...
    public RocksDBConsumeQueueOffsetTable.PhyAndCQOffset binarySearchInCQ(String topic, int queueId, long high, long low, long targetPhyOffset,
    ...
    private void buildCQKeyByteBuffer0(final ByteBuffer byteBuffer, final byte[] topicBytes, final int queueId, final long cqOffset) {
        byteBuffer.putInt(topicBytes.length).put(RocksDBConsumeQueueStore.CTRL_1).put(topicBytes).put(RocksDBConsumeQueueStore.CTRL_1).putInt(queueId).put(RocksDBConsumeQueueStore.CTRL_1).putLong(cqOffset);
        byteBuffer.flip();
    }
    ...
    private ByteBuffer buildDeleteCQKey(final boolean start, final byte[] topicBytes, final int queueId) {
        ...
        byteBuffer.putInt(topicBytes.length).put(RocksDBConsumeQueueStore.CTRL_1).put(topicBytes).put(RocksDBConsumeQueueStore.CTRL_1).putInt(queueId).put(start ? RocksDBConsumeQueueStore.CTRL_0 : RocksDBConsumeQueueStore.CTRL_2);
        byteBuffer.flip();
        return byteBuffer;
    }
}
