...
public class TransactionMetricsFlushService extends ServiceThread {
    ...
    private Broker brokerController;
    public TransactionMetricsFlushService(Broker brokerController) {
        ...
        while (!this.isStopped()) {
            try {
                if (System.currentTimeMillis() - start > brokerController.getBrokerConfig().getTransactionMetricFlushInterval()) {
                    ...
                    brokerController.getBrokerMessageService().getTransactionalMessageService().getTransactionMetrics().persist();
