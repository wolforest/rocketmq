        ...
        switch (consumeFromWhere) {
            case CONSUME_FROM_LAST_OFFSET: {
                ...
                if (lastOffset >= 0) {
                    ...
                    if (mq.getTopic().startsWith(MQConstants.RETRY_GROUP_TOPIC_PREFIX)) { // First start, no offset
                        result = 0L;
                    } else {
                        try {
            ...
            case CONSUME_FROM_TIMESTAMP: {
                ...
                if (lastOffset >= 0) {
                    ...
                    if (mq.getTopic().startsWith(MQConstants.RETRY_GROUP_TOPIC_PREFIX)) {
                        try {
                            result = this.mQClientFactory.getMQAdminImpl().maxOffset(mq);
                        } catch (MQClientException e) {
                            log.warn("Compute consume offset from last offset exception, mq={}, exception={}", mq, e);
                        ...
                        try {
                            long timestamp = DateUtils.parseDate(this.litePullConsumerImpl.getDefaultLitePullConsumer().getConsumeTimestamp(),
